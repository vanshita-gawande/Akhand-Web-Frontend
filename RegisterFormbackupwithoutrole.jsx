...
import { parsePhoneNumberFromString } from "libphonenumber-js";
import validator from "validator";
import { registerUser } from "../../api"; // <-- adjust path if your api file lives elsewhere
...
export default function RegisterForm({ onSuccess, onSwitch, onClose }) {
  ...
  // validation (keeps the original logic)
  const validateField = (field, value, allValues = values) => {
    let message = "";
    ...
    if (field === "password") {
      const pwErrors = checkPassword(value);
      if (Object.values(pwErrors).some((v) => v)) message = "Password does not meet all requirements";
    }

    if (field === "confirm" && value !== allValues.password) message = "Passwords do not match";
  ...
  const handleChange = (field, value) => {
    if (field === "mobile") {
      // keep digits-only for direct inputs (child phone input handles formatting)
      value = value.replace(/\D/g, "");
    }
    if (field === "name") {
      value = value.replace(/[^A-Za-z]/g, "");
  ...
  // mobile change handler used by PhoneField
  const handleMobileChange = (phone, country) => {
    const formatted = phone.startsWith("+") ? phone : `+${phone}`;
    const countryCode = country?.countryCode || "in";

    const parsed = parsePhoneNumberFromString(formatted, countryCode.toUpperCase());
    let nationalNumber = "";
    if (parsed) {
      nationalNumber = parsed.nationalNumber || "";
    }
    ...
    const requiredLen = phoneRules[countryCode.toUpperCase()] || 15;
    if (nationalNumber.length > requiredLen) {
      // block extra digits — no state update
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // keep original behaviour (res expected to contain token & user)
      if (res?.token) {
        localStorage.setItem("token", res.token);
        localStorage.setItem("user", JSON.stringify(res.user));
        localStorage.setItem("username", res.user.firstName || "User");
